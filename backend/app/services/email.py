      ...
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          <tr>
            <td style="background:linear-gradient(to right,{primary},{accent});padding:28px 40px;">
              <p style="margin:0;color:#ffffff;font-size:13px;letter-spacing:1px;text-transform:uppercase;font-weight:600;">RegEvent</p>
              <h1 style="margin:8px 0 0;color:#ffffff;font-size:22px;font-weight:700;line-height:1.3;">You won at {event_title}!</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:36px 40px 28px;">
...
def _build_prize_revoke_html(name: str, event_title: str, prize_title: str | None) -> str:
    ...
    accent = "#AACDDC"
    prize_line = (
        f"<p style='margin:0 0 16px;font-size:15px;color:#374151;'>Revoked prize: <strong>{prize_title}</strong></p>"
        if prize_title else ""
    )
    return f"""<!DOCTYPE html>
<html lang="en">
...
async def send_prize_revoke_email(to: str, name: str, event_title: str, prize_title: str | None = None) -> None:
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Your prize at {event_title} has been revoked"
    msg["From"] = smtp_from
    msg["To"] = to
    ...
    try:
        await aiosmtplib.send(msg, **send_kwargs)
        logger.info("[email] Prize revoke email sent to %s", to)
    except Exception:
...
async def send_winner_email(to: str, name: str, event_title: str, prize_title: str | None = None) -> None:
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"You won at {event_title}!"
    msg["From"] = smtp_from
    msg["To"] = to
    ...
    try:
        await aiosmtplib.send(msg, **send_kwargs)
        logger.info("[email] Winner email sent to %s", to)
    except Exception:
        logger.exception("[email] Failed to send winner email to %s", to)
